...
class AirflowConfigValidator(ABCMeta):
    @abstractclassmethod
...
class S3Validator(AirflowConfigValidator):
    ...
    def validate(self, config: AirflowConfigParser):
        is_enabled = config.getboolean(self.section, 's3_enabled')
        if is_enabled:
            remote_log_folder = config.get(self.section, 's3_remote_log_folder')
            encrypt_logs = config.getboolean(self.section, 's3_encrypt_logs')
            if not remote_log_folder:
                raise AirflowValidationException('')
